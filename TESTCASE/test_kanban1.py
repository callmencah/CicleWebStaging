from seleniumbase import SB
from seleniumbase import BaseCase


class CreateTeamTest(BaseCase):
    def test_create_team(sb):
        with SB(uc=True) as sb:
            # ----------------------------------------------------------------------------------------------------------
            # Login
            sb.open("https://staging.cicle.app/signin")
            sb.click('//*[@id="root"]/div/div[2]/div[2]/div[1]')
            sb.type('input[type="email"]', "lordula196")
            sb.click('//*[@id="identifierNext"]/div/button/div[1]')
            sb.type('input[type="password"]', "qwerty17;")
            sb.click('//*[@id="passwordNext"]/div/button/div[1]')
            sb.assert_text("Hello and welcome to Cicle!", "h1")
            sb.save_screenshot("Login_Success", "Custom_Screenshots")
            sb._print("Login Success")
            # ----------------------------------------------------------------------------------------------------------
            # Create Company
            sb.click(
                'img[src="data:image/png;base64,"]')
            sb.type('input[name="name"]', "Company1")
            sb.type('textarea[name="desc"]', "Company1")
            sb.click('h1:contains("Create Company")')
            sb.save_screenshot("Create_Company", "Custom_Screenshots")
            sb._print("Create Company Success")
            # ----------------------------------------------------------------------------------------------------------
            # Create Team
            sb.click("div#root div:nth-of-type(3) div:nth-of-type(4)")
            sb.click("div#root div:nth-of-type(3) div:nth-of-type(3) div:nth-of-type(2) div:nth-of-type(2) img")
            sb.type('input[name="name"]', "Team1")
            sb.type('textarea[name="desc"]', "Team1")
            sb.click('//*[@id="root"]/div[3]/div[3]/div[2]/div[2]/div[2]/button/div/h1')
            sb.save_screenshot("Create_Team", "Custom_Screenshots")
            sb._print("Create Team Success")
            # ----------------------------------------------------------------------------------------------------------
            # Navigate To Board
            sb.click('//*[@id="root"]/div[3]/div[4]/a/div/div[1]/p')
            sb.click('img[src="/static/media/board.82e692f2.png"]')
            sb.click("div.ListContainer_ListContainer__outerList__1PG0- > div > button > div > div")
            sb.save_screenshot("Navigate_To_Board", "Custom_Screenshots")
            sb._print("Success Navigate To Board")
            # ----------------------------------------------------------------------------------------------------------
            # Create BoardList
            sb.type('input[name="name"]', "Board1")
            sb.click("div.ListContainer_ListContainer__outerList__1PG0- div form div:nth-of-type(2) button div")
            sb.save_screenshot("Create_BoardList", "Custom_Screenshots")
            sb._print("Success Create BoardList")
            # ----------------------------------------------------------------------------------------------------------
            # Create BoardListSameName
            sb.click("div.ListContainer_ListContainer__outerList__1PG0- div:nth-of-type(2) button div")
            sb.type('input[name="name"]', "Board1")
            sb.click(
                "div.ListContainer_ListContainer__outerList__1PG0- div:nth-of-type(2) div form div:nth-of-type(2) button div")
            sb.save_screenshot("Create_BoardListSameName", "Custom_Screenshots")
            sb._print("Success Create BoardListSameName")
            # ----------------------------------------------------------------------------------------------------------
            # Create Card
            sb.click("div.ListContainer_ListContainer__outerList__1PG0- div div:nth-of-type(4) a")
            sb.type("input.form-control", "Card")
            sb.click(
                "div.ListContainer_ListContainer__outerList__1PG0- div div:nth-of-type(4) div:nth-of-type(3) button div")
            sb.refresh()
            sb.save_screenshot("Create_Card", "Custom_Screenshots")
            sb._print("Success Create Card")
            # ----------------------------------------------------------------------------------------------------------
            # Create Card
            sb.click("div.ListContainer_ListContainer__outerList__1PG0- div div:nth-of-type(4) a")
            sb.type("input.form-control", "Card1")
            sb.click(
                "div.ListContainer_ListContainer__outerList__1PG0- div div:nth-of-type(4) div:nth-of-type(3) button div")
            sb.refresh()
            # ----------------------------------------------------------------------------------------------------------
            # Edit name Card
            # sb.hover_and_click('//*[@id="listcard_content-0"]/div[1]/div/div/div[5]', '//*[@id="editCardMenuIcon"]')
            # sb.wait_for_element('//*[@id="editCardMenuButton"]/div[7]')
            # sb.click('//*[@id="editCardMenuButton"]/div[7]')
            # sb.click_xpath('/html/body/div[1]/div/div[3]/div/div/div[1]/div/div/div[3]/div/div/div/div/div[2]/div[7]')
            # sb.click("/html/body/div[1]/div/div[3]/div/div/div[1]/div/div/div[1]/div[2]/div[2]/div[3]/form/input[1]")
            #
            # sb.send_keys(
            #     "/html/body/div[1]/div/div[3]/div/div/div[1]/div/div/div[1]/div[2]/div[2]/div[3]/form/input[1]",
            #     "Card Edit")
            # # sb.click('//*[@id="root"]/div/div[3]/div/div/div[1]/div/div/div[1]/div[2]/div[2]/div[3]/div/div/button')
            # sb.save_screenshot("Change Name Private Card", "Custom_Screenshots")
            # sb._print("Success Change Name Private Card")
            # sb.sleep(20)
            # ----------------------------------------------------------------------------------------------------------
            # Change Card To Private Card
            sb.hover_and_click('//*[@id="listcard_content-0"]/div[1]/div/div/div[5]', '//*[@id="editCardMenuIcon"]')
            sb.wait_for_element('//*[@id="editCardMenuButton"]/div[7]')
            sb.click('//*[@id="editCardMenuButton"]/div[7]')
            sb.click('//*[@id="editCardMenuButton__ChangeAccess_text"]')
            sb.save_screenshot("Change Card To Private Card", "Custom_Screenshots")
            sb._print("Success Change Card To Private Card")
            sb.refresh()
            # ----------------------------------------------------------------------------------------------------------
            # Change Private Card To Public Card
            sb.hover_and_click('//*[@id="listcard_content-0"]/div[1]/div/div/div[5]', '//*[@id="editCardMenuIcon"]')
            sb.wait_for_element('//*[@id="editCardMenuButton"]/div[7]')
            sb.click('//*[@id="editCardMenuButton"]/div[7]')
            sb.click('//*[@id="editCardMenuButton__ChangeAccess_text"]')
            sb.save_screenshot("Change Change Private Card To Public Card", "Custom_Screenshots")
            sb._print("Success Change Private Card To Public Card")
            sb.refresh()
            # ----------------------------------------------------------------------------------------------------------
            # Navigate To Card Page
            sb.hover_and_click('//*[@id="listcard_content-0"]/div[1]/div/div/a', '//*[@id="listcard_content-0"]/div[1]/div/div/a')
            sb.wait_for_element('//*[@id="cardModal"]/div/div/div[1]/div[4]/div[2]/div/p')
            sb.save_screenshot("Navigate To Card Page", "Custom_Screenshots")
            sb._print("Success Navigate To Card Page")
            # ----------------------------------------------------------------------------------------------------------
            # Close Card Page
            sb.click('//*[@class="close"]')
            sb.save_screenshot("Close Card Page", "Custom_Screenshots")
            sb._print("Success Close Card Page")
            # ----------------------------------------------------------------------------------------------------------
            # Create Private Card
            sb.click("div.ListContainer_ListContainer__outerList__1PG0- div div:nth-of-type(4) a")
            sb.type("input.form-control", "Card")
            sb.type("input.form-control", "Private Card")
            sb.click('//*[@id="root"]/div/div[3]/div/div/div[1]/div/div/div[4]/div[2]/span/span[1]')
            sb.click(
                "div.ListContainer_ListContainer__outerList__1PG0- div div:nth-of-type(4) div:nth-of-type(3) button div")
            sb.refresh()
            sb.save_screenshot("Create_PrivateCard", "Custom_Screenshots")
            sb._print("Success Create Private Card")